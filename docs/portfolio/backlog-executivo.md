...
Disponibilizar um formato único de backlog executivo para comparar os três projetos do workspace:
...
| --- | --- | --- | --- |
| Sprint 1 | foundation, contratos e baseline de inicialização | foundation, migrations e baseline de API | foundation, baseline frontend/backend e revisão do contrato de mensagem |
| Sprint 2 | CRUDs core e fluxo de votação | endpoints core de agendamento | ingestão core e APIs de relatório |
| Sprint 3 | eventos, consistência de integração e idempotência | endurecimento de contrato e estabilidade de integração | idempotência, retries e integração dos relatórios no frontend |
| Sprint 4 | observabilidade e prontidão de release | observabilidade e prontidão para avaliação | observabilidade, hardening e relatório de entrega |
