...
public class AssociadoServiceImpl implements AssociadoService {
    ...
    @Override
    @Transactional
    public AssociadoDto criarAssociado(final AssociadoDto dto) {
    ...
    @Override
    @Transactional
    public AssociadoDto alterarAssociado(final AssociadoDto dto) {
        Associado entity = modelMapper.map(dto, Associado.class);
        // Valida os dados do dto antes de salvar
        if (!validarEmailAssociado(entity.getEmail())) {
            throw new IllegalArgumentException("Email inválido: " + dto.getEmail());
    ...
    @Override
    @Transactional
    public void removerAssociado(final Long idAssociado) {
    ...
    public AssociadoDto buscarAssociadoPorId(final Long id) {
        Associado entityIn = repository.findById(id).orElseThrow(() -> new EntityNotFoundException());
    ...
    @Override
    public AssociadoDto buscarAssociadoPorEmail(final String email) {
        if (!validarEmailAssociado(email)) {
            throw new IllegalArgumentException("false ");
        }

        Associado result = repository.findByEmail(email).orElseThrow(() -> new EntityNotFoundException());
        return modelMapper.map(result, AssociadoDto.class);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<AssociadoDto> buscarAssociadosPorNome(final String nome) {
    ...
    @Override
    public boolean validarEmailAssociado(final String email) {
