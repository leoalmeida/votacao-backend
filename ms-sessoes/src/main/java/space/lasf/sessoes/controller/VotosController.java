...
public class VotosController {
    ...
    public ResponseEntity<List<VotoDto>> pesquisarVotosSessao(@PathVariable final Long idSessao) {
    ...
    public ResponseEntity<Map<Long, VotoDto>> pesquisarVotosAssociado(@PathVariable final Long idAssociado) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<VotoDto> buscarVotoById(@PathVariable final Long id) {
        return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(votoService.buscarVotoPorId(id));
    }
    ...
    @PostMapping
        ...
        URI endereco =
                uriBuilder.path("/votos/{id}").buildAndExpand(novoVoto.getId()).toUri();
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> removerVoto(@PathVariable final Long id) {
